...
export default async function IntegrationsMarketingPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-brand-bg">
      <nav className="sticky top-0 z-40 border-b bg-white/95 backdrop-blur">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          ...
          <div className="hidden md:flex items-center gap-5 text-sm font-medium text-brand-text-muted">
            ...
            <Link href={`/${locale}/automation`} className="hover:text-brand-text-main">{tc('nav.automation')}</Link>
            <Link href={`/${locale}/pricing`} className="hover:text-brand-text-main">{tc('nav.pricing')}</Link>
          </div>
      ...
      <div className="container mx-auto max-w-3xl px-4 py-20">
        <h1 className="font-heading text-4xl font-bold text-brand-text-main mb-6">{t('pages.integrations.title')}</h1>
        <p className="text-lg text-brand-text-muted leading-relaxed mb-10">{t('pages.integrations.body')}</p>
